...
import com.david.facetec.FacetecManager;
import com.david.facetec.ParamsUtils;
...
public class PhotoIDMatchProcessor extends Processor implements FaceTecFaceScanProcessor, FaceTecIDScanProcessor {
    ...
    public PhotoIDMatchProcessor(String sessionToken, Context context, @NonNull Processor.SessionTokenErrorCallback sessionCallback) {
...
//        sampleAppActivity.setLatestSessionResult(sessionResult);
        ...
        if (sessionResult.getStatus() != FaceTecSessionStatus.SESSION_COMPLETED_SUCCESSFULLY) {
            ...
            sessionCallback.onError("onProcessingError", ParamsUtils.Companion.getZoomParams(sessionResult));
            return;
        }
        ...
        try {
            ...
            parameters.put("externalDatabaseRefID", FacetecManager.getLatestExternalDatabaseRefID());
...
//                    sampleAppActivity.setLatestServerResult(responseJSON);
                    ...
                    if (didSucceed) {
                        ...
                        sessionCallback.onSuccess("onProcessingComplete", ParamsUtils.Companion.getZoomParams(sessionResult));
                    } else {
...
//                    sampleAppActivity.setLatestServerResult(responseJSON);
                    ...
                    if (didSucceed) {
                        ...
                        sessionCallback.onSuccess("onProcessingComplete", ParamsUtils.Companion.getZoomIdParams(sessionResult, idScanResult));
                    } else {
                        // CASE:  In our Sample code, "success" being present and false means that the User Needs to Retry.
                        // Real Users will likely succeed on subsequent attempts after following on-screen guidance.
                        // Attackers/Fraudsters will continue to get rejected.
                    ...
                    Log.d("FaceTecSDKSampleApp", "Exception raised while attempting to parse JSON result.");
                    idScanResultCallback.cancel();
                    sessionCallback.onError("onProcessingError", ParamsUtils.Companion.getZoomIdParams(sessionResult, idScanResult));
                }
            }
